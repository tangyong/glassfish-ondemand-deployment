...
import org.glassfish.osgiondemanddeployment.OndemandDeploymentObrHandler;
...
public class OSGiDeployer implements Deployer<OSGiContainer, OSGiDeployedBundle> {
    ...
    public boolean prepare(DeploymentContext context) {
        ...
        if (params.origin.isDeploy()) {
            ...
            //TangYong Added: handling ondemand deploying bundle's dependencies using
            //OBR
    		...
    		if ((props != null) && ( props.containsKey("ondemand")) ) {
    			...
    			if (Boolean.valueOf(ondemandprop)){
    				...
    				OndemandDeploymentObrHandler ondemandObrHandler = null;
    				ServiceReference sr = bctx.getServiceReference("ondemandObrHandler");
    				if (sr == null){
    					ondemandObrHandler = new OndemandDeploymentObrHandler(bctx);
    					//Registering the ondemandObrHandler object into OSGi registry in order
        				//that Intercepted BundleContext can use it
        				bctx.registerService("OndemandDeploymentObrHandler", ondemandObrHandler, null);
    				}else{
    					ondemandObrHandler = (OndemandDeploymentObrHandler) bctx.getService(sr);
    				}
    								
    				if ( props.containsKey("obrurls") ){
    					String obrurls = props.getProperty("obrurls");
    					if (obrurls != null){
    						for (String s : obrurls.split("&")) {
    							try {
									ondemandObrHandler.addRepository(URI.create(s));
								} catch (Exception e) {
								}
    						}
    					}
    				...
    				try {
						Manifest manifest = context.getOriginalSource().getManifest();
						String symbolicName = manifest.getMainAttributes().getValue("Bundle-SymbolicName");
						String version = manifest.getMainAttributes().getValue("Bundle-Version");
						
						Bundle currentBundle = ondemandObrHandler.deploy(symbolicName, version);
						//Updating the currentBundle using bundle.update method
						InputStream input = new FileInputStream(file);
						currentBundle.update(input);
					} catch (Exception e) {
					}
    			}
    		}else{
